...
my @config_files
    = qw(/etc/iplant.superauthenticate.json  ./iplant.superauthenticate.json );
...
sub proxy {
    ...
    push(
        @opt_parameters,
        [   'proxy_user=s',
            "iPlant username to proxy",
            { 'default' => 'api_sample_user' }
        ]
    );
    ...
    if ( defined($nt) ) {
        ...
        print STDERR "Error establishing proxy for "
            . $opt1->{'proxy_user'} . "\n";
        exit kExitError;
